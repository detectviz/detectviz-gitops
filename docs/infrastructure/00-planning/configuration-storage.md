...
- [儲存磁碟規格](#1-儲存磁碟規格)
- [儲存架構規劃](#2-儲存架構規劃)
  - [ZFS 與 LVM-Thin 比較](#zfs-與-lvm-thin-比較)
- [Proxmox 儲存設置步驟](#2-proxmox-儲存設置步驟)
  - [NVMe 初始化與設定高效儲存池](#21-nvme-初始化與設定高效儲存池)
  - [檢查儲存池結構](#22-檢查儲存池結構)
  - [驗證說明](#23-驗證說明)
...
- **系統層（Proxmox 主機層級）**
  - **儲存設備**：SATA 512 GB
  - **掛載點**：`/`
  - **用途**：Proxmox 作業系統與核心服務，並於同一顆硬碟內設置備份資料區存放 ISO、模板、VM 備份等資料。
...
local             dir     active        98497780         3888900        89559332    3.95%
local-lvm     lvmthin     active       365760512         2121410       363639101    0.58%
nvme-vm       lvmthin     active      2000150528               0      2000150528    0.00%
```

### 2.3 驗證說明

> 儲存池均顯示 `active` 即代表設定完成且運作正常。
